                ...
                <h3 className="flex align-middle justify-between">
                  {edit ? <input value={taskInput} onChange={(event) => setTaskInput(event.target.value)} className="w-80 p-1"/> : <span className={`${task.completed?'line-through text-slate-400':''}`}>{task.text}</span>}
                    <span className="flex gap-3">
                      {edit? <><FaCheck onClick={() => submitNewTask(task._id,taskInput)} className="text-green-500"/> <FaTimes className="text-red-600" onClick={() => setEdit(!edit)} /></> : <><FaPencilAlt onClick={() => setEdit(!edit)} /> <FaTrash className="cursor-pointer" onClick={() => onDelete(task._id)}/></>}
                    </span>
                </h3>
              </section>
        </div>
